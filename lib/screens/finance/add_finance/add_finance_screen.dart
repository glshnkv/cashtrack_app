...
import 'package:cashtrack_app/widgets/action_button_widget.dart';
              ...
              children: [
                ...
                ActionButtonWidget(
                    text: 'Add Income', width: double.infinity, onTap: () {
                      if (valueController.text.isNotEmpty && int.tryParse(valueController.text) != null) {
                        FinanceModel newFinance = widget.finance;
                        newFinance.revenues += int.parse(valueController.text);
                        newFinance.history.insert(0, IncomeModel(value: int.parse(valueController.text), category: category, icon: 'assets/images/finance/${category.toLowerCase()}.svg'));
                        context.read<FinanceBloc>().add(AddFinanceEvent(oldFinance: widget.finance, newFinance: newFinance));
